...
import type { EmailAction, EmailRegistrationDecision } from "../email/models";
import type { LoginIdentifier } from "../identity/models";
import type { ProofBinding, ProofContinuationId, ProofPurpose } from "../proofs/models";
...
export interface EmailSubjectTable<Subject extends Table> extends EmailSubjectReadTable<Subject> {
  readonly decodeActionRequirement: (
    ...
    action: EmailAction,
  ) => Effect.Effect<AuthenticationRequirement, DrizzleMappingError>;
  readonly nextSecurityRevision?: (
    current: SecurityRevision,
  ) => Effect.Effect<SecurityRevision, DrizzleMappingError>;
...
> {
  ...
  }) => Effect.Effect<import("../email/models").EmailCredentialSnapshot, DrizzleMappingError>;
}
...
> {
  ...
  readonly decodeInstant: (native: unknown) => Effect.Effect<number, DrizzleMappingError>;
}
...
> {
  ...
  readonly allocateSubjectId?: Effect.Effect<NativeSubjectId, DrizzleMappingError>;
  readonly allocateSubjectIdSync?: () => NativeSubjectId;
  readonly decodeGeneratedId?: (
    rows: ReadonlyArray<unknown>,
  ) => Effect.Effect<NativeSubjectId, DrizzleMappingError>;
}
