...
/* oxlint-disable no-explicit-any -- private execution bridge; target wrappers retain payload, table, ID, and requirement types. */
import { Effect, Layer, Schema } from "effect";
import type { Statement } from "effect/unstable/sql/Statement";

import { LifecycleHooks } from "../hooks/LifecycleHooks";
import { PasskeyConfigurationError } from "../passkey/errors";
import * as M from "../passkey/models";
import { PasskeyManagementPersistence } from "../passkey/PasskeyManagementPersistence";
import { PasskeyManagementPolicy, PasskeyMethodPolicy } from "../passkey/policy";
...
import { completeEnrollment, issueEnrollment } from "./passkey-enrollment";
import {
  inspectRemove,
  listCredentials,
  removeCredential,
...
import {
  ...
  inspectRegistration,
  issueRegistration,
} from "./passkey-registration-write";
import { CurrentPasskeyTransaction, invariant, nonce, unavailable } from "./passkey-state";
import {
  capturedMapping,
  capturedService,
...
import { coordinateTransactionOwner } from "./transaction-execution";
  ...
  capturedService<PasskeyRegistrationWriter<R>>(
    {
      ...
      issueRegistration: (input, prepare) =>
        execute.run(
          Effect.gen(function* () {
            return yield* prepareValue(
              snapshotPasskeySync(M.PasskeyIssueDecision, yield* issueRegistration(mapping, input)),
              prepare,
            );
          }),
        ),
...
export const makeTargetPasskeyManagement = <M, RSetup>(
  database: any,
  source: PasskeyMappingSource<M, RSetup>,
  configuration: PasskeyTargetConfiguration,
) =>
...
export const makeTargetPasskeyRegistrationWriter = <M, R, RSetup>(
  database: any,
  source: PasskeyMappingSource<M, RSetup>,
  configuration: PasskeyTargetConfiguration,
) =>
