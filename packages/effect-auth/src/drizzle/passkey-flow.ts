/* oxlint-disable no-explicit-any -- private mapped-row kernel; public wrappers retain concrete tables. */
...
import type { PasskeyFlowState } from "./passkey-model";
import {
  releaseRegistrationCustody,
  scrubRegistrationIntent,
} from "./passkey-registration-custody";
import {
  ceremonyStorage,
  col,
  credentialStorage,
...
export const assertionPurposes = ["sign-in", "pending", "step-up", "action"] as const;
...
export const knownSubject = (ceremony: PasskeyCeremony): SubjectId | undefined =>
...
export const liveCondition = (mapping: any, ceremony: PasskeyCeremony, claim?: PasskeyClaim) => {
  ...
  return both(
    ...
    claim === undefined
      ? undefined
      : both(sql`${now} >= ${claim.claimedAtMillis}`, sql`${now} < ${claim.claimExpiresAtMillis}`),
  );
};

export const matchesAccess = (ceremony: PasskeyCeremony, access: PasskeyAccess) =>
...
export const readFlow = Effect.fn("passkey.readFlow")(function* (mapping: any, flowId: string) {
...
export const terminalFlow = Effect.fn("passkey.terminalFlow")(function* (
  mapping: any,
  read: FlowRead,
...
export const issueAssertion = Effect.fn("passkey.issueAssertion")(function* (
  mapping: any,
  ceremony: PasskeyCeremony,
  ...
  const subject = subjectId === undefined ? undefined : yield* readSubject(mapping.read, subjectId);
  ...
  if (sameCommand.rows.length !== 0 || sameFlow !== undefined) return { _tag: "Rejected" } as const;
...
export const contextAssertion = Effect.fn("passkey.contextAssertion")(function* (
  mapping: any,
...
export const claimAssertion = Effect.fn("passkey.claimAssertion")(function* (
  mapping: any,
  input: {
  ...
  if (read === undefined || read.state !== "Pending" || !matchesAccess(read.ceremony, input.access))
    return { _tag: "Rejected" } as const;
...
export const exactClaim = (read: FlowRead, claim: PasskeyClaim) =>
  read.state === "Claimed" &&
...
const evidenceMatches = (
  ...
  const ids = [
    ...new Set([...original.credentials.map((item) => item.credentialId), credential.credentialId]),
  ];

  const expected = {
    ...credential.revision,
    credentials: credential.revision.credentials.filter((item) => ids.includes(item.credentialId)),
  };
  ...
  return (
    ...
    selected !== undefined &&
    expected.credentials.some(
      (item) => item.credentialId === selected.credentialId && item.revision === selected.revision,
    )
  );
};
...
export const settleAssertion = Effect.fn("passkey.settleAssertion")(function* (
  mapping: any,
  claim: PasskeyClaim,
  outcome:
...
export const cleanupCeremonies = Effect.fn("passkey.cleanupCeremonies")(function* (
  mapping: any,
  ...
  for (const candidate of rows) {
    ...
    if (read.state === "Pending" || read.state === "Claimed") {
      const deadline =
        read.state === "Pending" ? read.ceremony.expiresAtMillis : read.claim!.claimExpiresAtMillis;
