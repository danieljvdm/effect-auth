/* oxlint-disable no-explicit-any -- private mapped-row kernel; public mappings retain concrete tables. */
import { sql } from "drizzle-orm";
import { Effect } from "effect";

import { type PasskeyCeremony, PasskeyCeremony as Ceremony } from "../passkey/models";
...
import {
  ...
  credentialKey as credentialKeyFor,
} from "./passkey-state";
import {
  authorizeAction,
  credentialRows,
...
export const enrollmentHandle = Effect.fn("passkey.enrollmentHandle")(function* (
  mapping: any,
  ...
  const absent = yield* owner.read(table.table, equal(table.table, { [table.handleKey]: hashed }), {
    limit: 1,
  });
...
export const issueEnrollment = Effect.fn("passkey.issueEnrollment")(function* (
  mapping: any,
  input: Issue,
) {
  const owner = yield* CurrentPasskeyTransaction;
  ...
  invariant(
    digest(Ceremony.fields.context, ceremony.context) ===
      digest(Ceremony.fields.context, { ...ceremony.context, authorization: input.authorization }),
  );
...
export const completeEnrollment = Effect.fn("passkey.completeEnrollment")(function* (
  mapping: any,
  input: Complete,
) {
  const owner = yield* CurrentPasskeyTransaction;
