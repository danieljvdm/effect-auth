...
import { ExternalIdentityMutation } from "../identity/ExternalIdentityMutation";
import type { ExternalIdentity, LoginIdentifier } from "../identity/models";
import { SubjectProvisioner, type SubjectProvisioningInput } from "../identity/SubjectProvisioner";
import { OAuthStateStore } from "../oauth/OAuthStateStore";
import type { OAuthState } from "../oauth/schema";
import {
...
export type { ChallengeConsumeDecision } from "../internal/AuthStoreDecisions";
export type { ConsumeDecision } from "../auth/ConsumeDecision";

export class DrizzleMappingError extends Schema.TaggedError<DrizzleMappingError>()(
  "DrizzleMappingError",
...
export interface InstantCodec<NativeInstant> {
  ...
  readonly decode: (native: NativeInstant) => Effect.Effect<DateTime.Utc, DrizzleMappingError>;
}
...
export interface ChallengeTableMapping<T extends Table> {
  ...
  ) => Effect.Effect<ChallengeRecord, DrizzleMappingError>;
}
...
export interface RegistrationTableMapping<T extends Table> {
  ...
  readonly decode: (
    row: InferSelectModel<T>,
  ) => Effect.Effect<RegistrationRecord, DrizzleMappingError>;
}
...
export interface SubjectIdCodec<NativeId> {
  readonly toNative: (id: SubjectId) => Effect.Effect<NativeId, DrizzleMappingError>;
  readonly toSubject: (id: NativeId) => Effect.Effect<SubjectId, DrizzleMappingError>;
...
> {
  ...
  readonly subject: {
    ...
    readonly allocateId?: Effect.Effect<NativeId, DrizzleMappingError>;
    ...
    ) => Effect.Effect<NativeId, DrizzleMappingError>;
  };
  readonly identifier: {
