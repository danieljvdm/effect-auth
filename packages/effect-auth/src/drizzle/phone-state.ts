/* oxlint-disable no-explicit-any -- native kernel; public wrappers retain exact consumer table/ID types. */
...
import { PhoneOtpUnavailable, PhoneCredentialSnapshot } from "../phone/models";
import type { PhoneMutation } from "../phone/PhonePersistence";
import type { SubjectId } from "../Schema";
import { AuthenticationRequirement, SecurityRevision } from "../sessions/models";
import { digest, randomId } from "../totp/crypto";
...
export const admitPhone = Effect.fn("PhoneNative.admit")(function* (
  ...
  if (accepted)
    for (const update of updates)
      yield* stateWrite(mapping, update.key, { window, count: update.count + 1 }, update.observed);
  yield* stateWrite(
    mapping,
    key,
    {
