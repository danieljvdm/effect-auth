...
import type { EmailAction, EmailCredentialSnapshot } from "../email/models";
import {
...
export const makeSqlEmailSignInTargets = Effect.fn("makeSqlEmailSignInTargets")(function (
  database: EmailSqlDatabase,
  mapping: SignInMapping,
  configuration: EmailSqlConfiguration,
...
export const currentAddress = Effect.fn("DrizzleEmail.currentAddress")(function* (
  ...
  if (mapping.addressCardinality === "single" && input.sourceCredentialId === undefined) {
    ...
    if (rows.some((row: any) => mapping.credential.isActiveStatus(row[mapping.credential.status])))
      eligible = false;
  }
...
export const snapshotEmailMutation = Effect.fn("DrizzleEmail.snapshotMutation")(function* (
...
const applyMutation = Effect.fn("DrizzleEmail.applyMutation")(function* (
  ...
  const targetAuthority = yield* selectRows(
    database
      .select()
      .from(mapping.authorityCredential.table)
      .where(and(eq(a.subjectId, current.nativeSubjectId), eq(a.credentialId, targetCredentialId)))
      .limit(1),
    false,
  );
...
export const makeSqlEmailAddressPersistence = Effect.fn("makeSqlEmailAddressPersistence")(
  function* (
    database: EmailSqlDatabase,
    mapping: AddressMapping,
    configuration: EmailSqlConfiguration,
...
const mutateAddress = Effect.fn("Drizzle.mutateAddress")(function* <A>(
  ...
  const nextRevision = yield* nextSecurityRevision(
    mapping,
    configuration.mode,
