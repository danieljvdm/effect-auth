/* oxlint-disable no-explicit-any -- private mapped-row kernel; public descriptors preserve concrete tables and native identities. */
...
import type { PasskeyActionAuthorization, PasskeyCeremony } from "../passkey/models";
import {
  PasskeyBegin,
  PasskeyCredential,
...
import { PasskeyManagementPolicy } from "../passkey/policy";
import { snapshotPasskeySync } from "../passkey/snapshot";
import { TokenDigest, type SubjectId } from "../Schema";
...
export const jsonStorage = <S extends Schema.Codec<unknown, unknown, never, never>>(schema: S) => {
...
export const digest = <S extends Schema.Codec<unknown, unknown, never, never>>(
  schema: S,
  value: S["Type"],
) =>
  TokenDigest.make(
...
export const currentSubject = Effect.fn("passkey.currentWriteSubject")(function* (
  mapping: any,
  subjectId: SubjectId,
) {
  ...
  const factors = (yield* owner.read(
    ...
    { limit: 64, columns: mappedColumns(factor), orderBy: col(factor.table, factor.credentialId) },
  )).rows;
...
export const credentialRows = Effect.fn("passkey.writeCredentialRows")(function* (
  mapping: any,
...
export const summary = (mapping: any, row: any) => {
  const table = mapping.read.credential;
...
export const ownedCredential = Effect.fn("passkey.ownedWriteCredential")(function* (
  mapping: any,
  subject: WriteSubject,
  row: any,
) {
...
export const managementPolicy = (mapping: any, subject: WriteSubject) =>
  snapshotPasskeySync(
    PasskeyManagementPolicy,
    mapping.write.policy.management(copiedRow(subject.row)),
  );

export const metadataAllowed = Effect.fn("passkey.metadataAllowed")(function* (
  mapping: any,
  subject: WriteSubject,
) {
  const owner = yield* CurrentPasskeyTransaction;
...
export const enrollmentDigest = (ceremony: PasskeyCeremony) => {
...
export const removeDigest = (moduleId: string, commandId: string, credential: PasskeyCredential) =>
  digest(
    Schema.Tuple([
      PasskeyModuleId,
...
export const authorizeAction = Effect.fn("passkey.authorizeWriteAction")(function* (
  mapping: any,
  subject: WriteSubject,
  ...
  const current = snapshotPasskeySync(
    PasskeyRequirement,
...
export const invalidationMatches = (
  mapping: any,
...
export const invalidate = Effect.fn("passkey.invalidateCredentialChange")(function* (
  mapping: any,
  subject: WriteSubject,
...
export const insertCredential = Effect.fn("passkey.insertCredential")(function* (
  mapping: any,
  subject: WriteSubject,
  ceremony: PasskeyCeremony,
  verified: any,
  ...
  const credential = snapshotPasskeySync(PasskeyCredential, {
    ...
    revision: {
      ...
      credentials: [
        ...
        { credentialId: id, revision: PasskeyRevision.fields.securityRevision.make(owner.marker) },
      ],
    },
    active: true,
