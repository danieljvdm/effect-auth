...
import type { PasskeyUnavailable } from "../passkey/errors";
import type {
...
import type { PreparePasskeyCommit } from "../passkey/PasskeyPersistence";
import type { PasskeyManagementPolicy, PasskeyMethodPolicy } from "../passkey/policy";
...
> {
  ...
  readonly policy: {
    ...
    readonly remainingSignIn: (subjectId: N, removedCredentialId: string) => SQL;
  };
}
...
> {
  ...
  /** Must match the installed session strategy. Subject revision is always changed; immediate sessions must consult that authority or be updated here. */
