...
import type { ProofRecord } from "../proofs/ProofPersistence";
...
export interface ProofRequestTable<Request extends Table> {
  ...
  ) => Effect.Effect<ProofRequestReceipt, DrizzleMappingError>;
}
...
export interface ProofGenerationTable<Generation extends Table> {
  ...
  ) => Effect.Effect<ProofRecord, DrizzleMappingError>;
  readonly decodeBinding: (
    row: InferSelectModel<Generation>,
  ) => Effect.Effect<ProofBinding, DrizzleMappingError>;
}
...
export interface ProofContinuationTable<Continuation extends Table> {
  ...
  ) => Effect.Effect<ProofContinuationRecord, DrizzleMappingError>;
}
...
> extends ProofConstraintClassifier {
  ...
  readonly allocateVersion?: Effect.Effect<ProofVersion, DrizzleMappingError>;
