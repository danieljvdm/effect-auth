/* oxlint-disable no-explicit-any -- private mapped-row kernel; public descriptors retain concrete table types. */
import { sql } from "drizzle-orm";
import { Effect, Schema } from "effect";

import { PasskeyCredentialSummary, PasskeyRemoved } from "../passkey/models";
import type { PasskeyManagementPersistence } from "../passkey/PasskeyManagementPersistence";
import { lockAdmission } from "./passkey-admission";
...
const access = Effect.fn("passkey.managementAccess")(function* (
  ...
  if (moduleId !== mapping.moduleId || (yield* readModule(mapping)) === undefined) return undefined;
...
export const listCredentials = Effect.fn("passkey.listCredentials")(function* (
  mapping: any,
  ...
  const eligible = rows.filter(
    (row) => input.cursor === undefined || row[mapping.read.credential.credentialId] > input.cursor,
  );
...
export const renameCredential = Effect.fn("passkey.renameCredential")(function* (
  mapping: any,
...
export const inspectRemove = Effect.fn("passkey.inspectRemove")(function* (
  mapping: any,
  ...
  const credential = row === undefined ? undefined : yield* ownedCredential(mapping, subject, row);
...
export const removeCredential = Effect.fn("passkey.removeCredential")(function* (
  mapping: any,
  input: Parameters<Port["remove"]>[0],
) {
  const subject = yield* access(mapping, input.moduleId, input.credential.revision.subjectId, true);
  ...
  if (
    ...
    return { _tag: "Rejected" } as const;
  const owner = yield* CurrentPasskeyTransaction;
  const remaining = mapping.write.policy.remainingSignIn(subject.nativeId, id);
